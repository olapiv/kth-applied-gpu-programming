 ...
 * The input block of the image is given by the top-left corner,
 * not the center of the block (the target pixel).
*/
//    cpu_applyFilter(&image[offset_t + w], width, sobel_x, 3);
...
// __device__ float handle_shared_memory(float *image, int offset_t, int index_x, int index_y, int width) {
//
...
int main(int argc, char **argv)
{
    ...
    // Step 2: Apply a 3x3 Gaussian filter
    {
        ...
        // Launch the GPU version
        gettimeofday(&t[0], NULL);
        gpu_gaussian<<<dim3(bitmap.width/BLOCK_SIZE +1, bitmap.height/BLOCK_SIZE +1, 1), dim3(BLOCK_SIZE, BLOCK_SIZE, 1)>>>(bitmap.width, bitmap.height,
    ...
    // Step 3: Apply a Sobel filter
    {
        ...
        // Launch the GPU version
        gettimeofday(&t[0], NULL);
        gpu_sobel<<<dim3(bitmap.width/BLOCK_SIZE +1, bitmap.height/BLOCK_SIZE +1, 1), dim3(BLOCK_SIZE, BLOCK_SIZE, 1)>>>(bitmap.width, bitmap.height,
